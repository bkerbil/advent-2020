...
(defn one-cycle
  ...
  (let [width (count (first (first space)))
    ...
    (loop [c coordinates
      ...
      (if (empty? c)
        result
        (let [coordinate (first c)
              updated-neighbours (map #(update-coordinates coordinate %) neighbours)
              [x y z] coordinate
              state (get-in space [z y x] 0)
              neighbours-active (->> updated-neighbours
                                     (map (fn [[a b c]] (get-in space [c b a] 0)))
                                     (reduce +))]
          (cond
            (and (= state 1) (>= 3 neighbours-active 2)) (recur (rest c) (assoc-in result [z y x] 1))
            (= state 1) (recur (rest c) (assoc-in result [z y x] 0))
            (and (= state 0) (= neighbours-active 3)) (recur (rest c) (assoc-in result [z y x] 1))
            :default (recur (rest c) (assoc-in result [z y x] 0))))))))
